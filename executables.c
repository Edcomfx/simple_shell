...
/**
  * print_prompt - Prints d prompt to the user
  */

void print_prompt(void)
{
	if (isatty(STDIN_FILENO))
		write(STDOUT_FILENO, "edshell$ ", 9);
}

/**
  * read_input - reads d input from the users
  *
  * Return: Charcter variable to the program
  */
...
/**
  * execute_command - executes d input from the buffer
  * @input:  argument from the buffer
...
/**
  ...
  *
  * Return: num of the items tokenized
  */
...
/**
  * handle_builtin_commands - handle all d built in commands
